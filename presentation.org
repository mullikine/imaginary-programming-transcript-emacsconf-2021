...
The =ilambda.el= primitives are =ieval=, =imacro= and =idefun=. They currently only a language model as a
parameter, but in future this will be an =sNFT= though a =semiosis protocol=.
#+latex: }
